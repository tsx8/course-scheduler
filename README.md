...
├── main.go
├── package.json   # repo 级脚本，仅保留 `build:solver`
├── wails.json
└── AGENTS.md
```
...
该命令会在 `solver/` 子项目中使用 `uv` 环境执行 PyInstaller，把 `solver/solver.py` 构建到 `solver/dist/solver.exe`，并由 Go/Wails 直接以该目录作为 sidecar 单一来源。
...
- 求解器 / 数据模型改动后运行 `npm run build:solver`
